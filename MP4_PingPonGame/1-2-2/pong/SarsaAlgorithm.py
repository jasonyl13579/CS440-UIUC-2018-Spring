...
class SarsaAlgorithm(object):
    ...
    def learning(self,state_old,state_new,reward,action,action_new):
        QNext=self.qValue.get((state_new,action_new),0.0)
        #max([self.qValue.get((state_new,action),0.0) for action in self.actionChoices])
        self.Sarsa(reward,reward+self.gammaValue*QNext,state_old,action)