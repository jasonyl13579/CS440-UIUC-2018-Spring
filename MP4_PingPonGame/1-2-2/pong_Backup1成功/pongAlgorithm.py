...
class pongAlgorithm(object):
    def __init__(self):
        ...
        self.SarsaAlgorithm=SarsaAlgorithm(self.ALPHA_VALUE,self.GAMMA_VALUE,self.TOLERANCE_THRESHOLD)    
    ...
    def updateFunction(self):
        ...
        currentReward=0.0
        gameState=self.updateCondition()
        action=self.SarsaAlgorithm.actionChoice(gameState)
        
        if self.termination:
            currentReward=-1000.0
            if self.previousState is not None:
                self.SarsaAlgorithm.learning(self.previousState,self.gameState,currentReward,self.previousAction,action)
            self.previousState=None
            self.bouncingBall=bouncingBall()
            self.movingPaddle=movingPaddle('QLearning')
            self.termination=False
        ...
        #action=self.SarsaAlgorithm.randomChoice()
    ...
    def readWeight(self):
        with open('QValue.txt','r') as f:
            for index in f:
                ...
                self.SarsaAlgorithm.qValue[((int(index[0]),int(index[1]),int(index[2]),int(index[3]),int(index[4])),int(index[5]))]=float(index[6])
