...
class pongAlgorithm(object):
    def __init__(self):
        ...
        self.movingPaddleUser=movingPaddle('User')
    ...
    def boundaryCondition(self):
        if self.bouncingBall.ball_x>self.movingPaddle.paddle_x:
            if self.bouncingBall.ball_y>self.movingPaddle.paddle_y and self.bouncingBall.ball_y<self.movingPaddle.paddle_y+self.movingPaddle.paddle_height:
                ...
                self.ending()
        if self.bouncingBall.ball_x<self.movingPaddleUser.paddle_y and self.bouncingBall.ball_y<self.movingPaddleUser.paddle_y+self.movingPaddleUser.paddle_height:
            self.bouncingBall.successfulCollisionUser()
            self.successCount+=1
            self.currentScore+=1
        else:
            self.terminationUser=True
            self.ending()
            self.successCount+=1
