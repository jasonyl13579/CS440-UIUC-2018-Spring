...
def plotGraph():
    with open('QValue.txt','w+') as f:
        for valuePair in pongGame.QLearningAlgorithm.qValue.keys():
            f.write(str(valuePair[0][0])+' '+str(valuePair[0][1])+' '+str(valuePair[0][2])+' '+str(valuePair[0][3])+' '+str(valuePair[0][4])+' '+str(int(float(valuePair[1])))+' '+str(pongGame.QLearningAlgorithm.qValue[valuePair])+'\n')
...
def plotPaddleUser(paddlePosition,pongGame):
	paddlePosition.y=resizeWall(pongGame.movingPaddleUser.paddle_y)
	pygame.draw.rect(DISPLAYSURF,BLACK_COLOR,paddlePosition)    
def plotWall(walls):
...
if __name__=='__main__':
	if algorithmChoice == 'practicing':
		...
		paddleUser_x=resizeWall(0.0)
		paddleUser_y=resizeWall(pongGame.movingPaddleUser.paddle_y)
		paddleUser_height=resizeWall(pongGame.movingPaddleUser.paddle_height)
		paddleUser=pygame.Rect(paddleUser_x,paddleUser_y,THICKNESS,paddleUser_height)

		wall_x=resizeWall(0.0)
		...
		while True:
			...
			plotWall(wall)
			plotPaddle(paddle,pongGame)
			plotPaddleUser(paddleUser,pongGame)
