...
class pongAlgorithm(object):
    ...
    def ending(self):
        if len(self.overallScores)>=10000:
        ...
        if self.roundCount%10000==0:
            totalScore=float(sum(self.overallScores))/10000.0
            self.roundRecord.append(self.roundCount)
            self.totalScoresRecord.append(self.totalScore)
        if self.RESTRICTION:
            if self.roundCount==ROUND_TOTAL:
                self.finish=True
        if self.OPTIMIZAION:
            if totalScore>9.0:
                self.finish=True
    ...
    def updateFunction(self):
        ...
        gameState=updateCondition()
        currentReward=0.0
        
        if self.termination:
            reward=-10000.0
            if self.previousState is not None:
                self.QLearningAlgorithm.learning(self.previousState,self.gameState,reward,self.previousAction)
            self.previousState=None
            self.bouncingBall=bouncingBall()
            self.movingPaddle=movingPaddle('QLearning')
            self.termination=False
        ...
        if self.collisionFlag:
            ...
            reward=10000.0
        if self.previousState is not None:
            self.QLearningAlgorithm.learning(self.previouState,self.gameState,reward,self.previousAction)
        gameState=self.updateCondition()
        action=self.QLearningAlgorithm.actionChoice(gameState)
        
        self.previousState=gameState
        self.previousAction=action
        self.movingPaddle.updateFunction(action)
