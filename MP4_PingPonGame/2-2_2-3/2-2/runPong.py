...
algorithmChoice = 'Animation'
pongGame=pongAlgorithm(algorithmChoice)
#pongGame.readWeight()p
...
#	elif algorithmChoice=='game':
	if algorithmChoice == 'Animation':
		...
		pygame.quit()
		sys.exit(0)
	elif algorithmChoice == 'Statistics':
		while not pongGame.finish:
			pongGame.updateFunction()
		avg = sum(pongGame.scores) / pongGame.ROUND_TOTAL
		x = np.arange(0, pongGame.ROUND_TOTAL)
		plt.plot(x, pongGame.scores)
		plt.xlabel('Game')
		plt.ylabel('Score')
		plt.title('Average score : ' + str(avg))
		print("Average scores of " + str(pongGame.ROUND_TOTAL) + " games : " + str(avg))
