...
#    for row in dA:
#        for col in range(len(row)):
#            row[col] = np.dot(dZ[index_row], W[col])
#        index_row += 1
#    index_row = 0
#    for row in dW:
#        for col in range(len(row)):
#            row[col] = np.dot(A[:, index_row], dZ[:, col])
#        index_row += 1
#    index_row = 0
#    for row in dB:
#        for col in range(len(row)):
...
#W = np.array(W)
#dZ = np.array(dZ)
#b = np.zeros((len(A),len(W[0])))
#b[:,0] = 0.699479275318
#b[:,1] = 0.297436950855
#b[:,2] = 0.813797819702
#b[:,3] = 0.396505740847
#
#(dA, dW, dB) = affineBackward(dZ, W, A, b)
#
#diff_dA = dA_t-dA
#diff_dW = dW_t-dW
#diff_dB = dB_t-dB[0,:]
