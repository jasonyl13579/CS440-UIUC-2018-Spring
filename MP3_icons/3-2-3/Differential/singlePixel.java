...
public class singlePixel
{
	...
	private double learningRate=0.01
			;
	private boolean randomWeights = true;
			...
			for(int i = 0; i < dataset.size(); i++) {
				...
				if(maxNum != trainLabel.get(i)) {
					if (!differential) {
						...
						for(int num = 0; num < 10; num++) {
							if (trainLabel.get(i) == num) y = 1; 
							else y = 0;
							double exp_outcome =  Math.exp(outcome[num]);
							for(int k = 0; k < rowNumber*colNumber; k++) {
								double first = ( exp_outcome*(float)(dataset.get(i).charAt(k)-'0'))*p_sum;
								double second =  exp_outcome * (float)(dataset.get(i).charAt(k)-'0') * exp_outcome;
								e_differential = 2 * (p[num] - y) * ( (first - second)/(p_sum * p_sum));
								weights[num][k] -= learningRate * e_differential;
							}
