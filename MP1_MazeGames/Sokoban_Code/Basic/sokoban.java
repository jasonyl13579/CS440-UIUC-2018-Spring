...
public class sokoban
{
	...
	private float value;
	public sokoban(int[][] array,int rowNumber,int colNumber,point currentPosition,Set<point> storagePoints,Set<point> boxPoints,Set<point> finishPoints, int heuristicChoice)
	{
		this.array=array;
	...
	public float calculateHeuristic(sokobanState state)
	{
		...
		if(heuristicChoice==1)
		{
			for(Iterator<point> itBox=state.getBoxPoints().iterator();itBox.hasNext();)
			{
				...
				for(Iterator<point> itStorage=storagePoints.iterator();itStorage.hasNext();)
				{
					...
					if(ManhattanDistance<minimumCost)
					{
		...
		else if(heuristicChoice==2)
		{
			...
			heuristic=heuristic-finishPoints.size();
